...
async function clearAllQueues() {
    ...
    for (const qDef of queuesToClear) {
        ...
        try {
            ...
            queue = new bullmq_1.Queue(qDef.name, { connection: qDef.connection });
            // Set a timeout for the obliterate operation
            const obliteratePromise = queue.obliterate({ force: true });
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error(`Obliterate operation timed out for queue ${qDef.name}`)), 8000) // Increased to 8s
            );
