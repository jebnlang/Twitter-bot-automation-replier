...
const LOG_FILE_PATH = process.env.LOG_FILE_PATH || 'twitter_activity_log.csv';
...
// Helper function to escape CSV data and ensure it's quoted
...
async function processLogEntryJob(job: Job) {
  ...
  try {
    await fs.appendFile(LOG_FILE_PATH, csvRow, 'utf8');
    console.log(`Logger Agent: Appended log entry for Post URL ${postUrl} to ${LOG_FILE_PATH}`);
  } catch (error) {
    console.error(`Logger Agent: Error appending to log file ${LOG_FILE_PATH}:`, error);
...
async function startLoggerAgent() {
  await ensureLogFileHasHeader(); // Check and create header if needed before worker starts
